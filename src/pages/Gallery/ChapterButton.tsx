...
export const ChapterButton: React.FC<ChapterButtonProps> = ({
  ...
  return (
    <button
      ...
      title="选择章节"
    >
      <p className="w-full pb-2 text-lg text-gray-800 dark:text-white dark:text-opacity-80">
        Chapter {index + 1}
      </p>
      ...
      {chapterStatus !== null && (
        <>
          {chapterStatus.exerciseCount > 0 ? (
            ...
            <>
              <p className="pt-1 text-xs font-medium text-gray-600 dark:text-white dark:text-opacity-60">
                暂未练习
              </p>
            </>
          )}
